...
bool MainWindow::IsMinimised() const
{
...
LRESULT MainWindow::HandleMsg( HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam )
{
	...
	case WM_INPUT:
	{
		...
		// need to implement a function in mouse class to access
		// as in accumulate deltas
	...
	case WM_MOUSEMOVE:
	{
		...
		if( x >= 0 && x < Graphics::WINDOW_WIDTH &&
			y >= 0 && y < Graphics::WINDOW_HEIGHT )
		{
		...
		else
		{
			...
			{
				x = std::max( 0,x );
				x = std::min( int( Graphics::WINDOW_WIDTH ) - 1,x );
				y = std::max( 0,y );
				y = std::min( int( Graphics::WINDOW_HEIGHT ) - 1,y );
				mouse.OnMouseMove( x,y );
			}
			else
			{
	...
	case WM_MOUSEWHEEL:
	{
		...
		mouse.wheelMoved = true;

		if(DELTA > 0)
		{
			mouse.OnWheelUp( x, DELTA );
		}
		else
		{
			mouse.OnWheelDown( x, DELTA );
		}
